...
// API Request Methods
import { requestListProductClasess } from '@/api/ADempiere/fields/search/product.ts'
...
export default defineComponent({
  ...
  setup(props) {
    const ATTRIBUTE_KEY = 'product_class_id'
    ...
    function loadProductClasses(isShowList) {
      if (!isShowList) {
        return
      }
      requestListProductClasess({
        pageSize: 100
      })
        .then(response => {
