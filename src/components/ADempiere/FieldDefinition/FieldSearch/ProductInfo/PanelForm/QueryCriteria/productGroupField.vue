...
// API Request Methods
import { requestListProductGroups } from '@/api/ADempiere/fields/search/product.ts'
...
export default defineComponent({
  ...
  setup(props) {
    const ATTRIBUTE_KEY = 'product_group_id'
    ...
    function loadProductGroups(isShowList) {
      if (!isShowList) {
        return
      }
      requestListProductGroups({
        pageSize: 100
      })
        .then(response => {
