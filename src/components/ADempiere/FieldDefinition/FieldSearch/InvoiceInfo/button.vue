...
<template>
  ...
  >
    ...
    <el-button slot="reference" class="button-search" :disabled="isDisabled">
...
import PanelGeneralInfoSearch from './PanelForm'
  ...
  setup(props) {
    ...
    const uuidForm = computed(() => {
      if (!isEmptyValue(props.parentMetadata.containerUuid)) {
        return (
          props.parentMetadata.columnName +
          '_' +
          props.parentMetadata.containerUuid
        )
      }
